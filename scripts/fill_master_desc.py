# -*- coding: utf-8 -*-
"""
Parse คำอธิบายรายวิชา (ป.โท) จาก มคอ.2 PDF (กศ.ม. เทคโนโลยีและสื่อสารการศึกษา)
ฟอนต์ THSarabunPSK ใช้ Private Use Area สำหรับวรรณยุกต์ -> remap กลับเป็น Unicode
เติม desc_th / desc_en ลงในไฟล์ src/content/courses/m-*.md (จับคู่ด้วยรหัสวิชา)
"""
import re, pathlib, pdfplumber

ROOT = pathlib.Path(__file__).resolve().parents[1]
PDF = ROOT / "scripts" / "ma.pdf"
OUT = ROOT / "src" / "content" / "courses"
PAGES = range(40, 53)  # หมวด 3.1.5 คำอธิบายรายวิชา
...
for b in blocks:
    body = b["lines"]
    ci = next((i for i, x in enumerate(body) if CREDIT.match(x)), None)
    if ci is None:
        continue
    post = body[ci + 1:]
    runs = []
